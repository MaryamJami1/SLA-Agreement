...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($_POST['action'] ?? '') {
            case 'create':
                flash('ok', 'Added venue “' . create_venue($pdo, $admin, $_POST['name'] ?? '', $_POST['sort_order'] ?? '0') . '”.');
                break;
            case 'update':
                update_venue($pdo, $admin, $id, $_POST['name'] ?? '', isset($_POST['is_active']), $_POST['sort_order'] ?? '0');
